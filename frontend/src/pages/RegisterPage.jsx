...
const Register = () => {
  ...
  const validateStep = () => {
    if (step === 1) {
      ...
      const { country, age_group, gender, travel_companion, travel_style } = formData;
      if (!country || !age_group || !gender || !travel_companion || !travel_style) {
  ...
  const activityOptions = ["Swimming", "Hiking", "Shopping", "Photography", "Nature", "Museums", "Educational Tours", "Rafting", "Cultural Activities","Animal Watching", "Surfing", "Cultural Tours", "Sunbathing", "Farm Tours"];
    ...
    >
      ...
      {step === 2 && (
        <>
          ...
          <div>
            <label className="block font-medium mt-4 mb-2">Travel Companion</label>
          ...
          <div>
            <label className="block font-medium mt-4 mb-2">Travel Style</label>
            <div className="flex flex-wrap gap-4">
              {["Backpacker", "Luxury", "Balanced", "Adventurous", "Relaxed"].map((type) => (
                <label key={type} className="flex items-center gap-2 text-sm">
                  <input
                    type="radio"
                    name="travel_style"
                    value={type}
                    checked={formData.travel_style === type}
                    onChange={handleChange}
                    required
                  />
                  {type}
      ...
      {step === 3 && (
        <>
          ...
          <div>
            <label className="block font-medium mt-4 mb-2">Preferred Activities</label>
            <div className="flex flex-wrap gap-2">
      ...
      {step === 4 && (
        <div className="space-y-2 text-sm">
          ...
          <p><strong>Age Group:</strong> {formData.age_group}</p>
          <p><strong>Gender:</strong> {formData.gender}</p>
          <p><strong>Travel Companion:</strong> {formData.travel_companion}</p>
          <p><strong>Travel Style:</strong> {formData.travel_style}</p>
          <p><strong>Locations:</strong> {formData.location_types.join(", ")}</p>
          <p><strong>Activities:</strong> {formData.preferred_activities.join(", ")}</p>
