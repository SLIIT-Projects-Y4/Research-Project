  ...
  try {
    ...
    return res.json({
      ...
      data: {
        ...
        country: user.country || null,               // not used by ML
...
exports.updatePreferences = async (req, res) => {
  ...
  if ("travel_style" in updates) updates.travel_style = String(updates.travel_style).trim(); // new
  if ("budget" in updates) updates.budget = String(updates.budget).trim();                   // new
  ...
  try {
    ...
    // onboarding completeness is still based on the core 4 only
    const hasCoreFour =
      user.age_group &&
      user.gender &&
      user.travel_companion &&
